...
import { FaHospitalSymbol, FaCalendarPlus, FaUserMd, FaNotesMedical, FaStar, FaStethoscope,
    FaHeartbeat, FaChild, FaBrain, FaTooth, FaPills, FaSyringe, FaLungs, FaClinicMedical
} from 'react-icons/fa'
function LandingPage({ onLoginClick, user, onLogout, appointments, addAppointment, doctors }) {
        // Hàm format ngày ISO yyyy-MM-dd
    ...
    // Map tên chuyên khoa sang icon (fallback generic icon)
    const getIconForSpecialty = (spec) => {
        if (!spec) return <FaUserMd />;
        const s = spec.toLowerCase();
        if (s.includes('tim') || s.includes('tim mạch') || s.includes('cardio')) return <FaHeartbeat />;
        if (s.includes('nhi')) return <FaChild />;
        if (s.includes('thần kinh') || s.includes('thần') || s.includes('não')) return <FaBrain />;
        if (s.includes('da') || s.includes('da liễu') || s.includes('da-lieu')) return <FaNotesMedical />;
        if (s.includes('tai') || s.includes('mũi') || s.includes('họng') || s.includes('tai mũi họng')) return <FaClinicMedical />;
        if (s.includes('răng') || s.includes('hàm') || s.includes('nha')) return <FaTooth />;
        if (s.includes('dược') || s.includes('thuốc') || s.includes('dược')) return <FaPills />;
        if (s.includes('tiêm') || s.includes('vaccin') || s.includes('vaccine')) return <FaSyringe />;
        if (s.includes('hô') || s.includes('phổi') || s.includes('hô hấp')) return <FaLungs />;
        // default
        return <FaUserMd />;
    };
    ...
    const handleBook = (e) => {
        e.preventDefault();
        const formattedDate = formatDateISO(bookingData.date);
        const newApp = {
            patientId: user.patientId || user.id,
            patientName: user.full_name,
            doctor_id: selectedDoctor.id,
            time_booked: `${formattedDate} (${bookingData.shift})`,
        ...
        alert(`Đặt lịch thành công!\nBác sĩ: ${selectedDoctor.name}\nThời gian: ${bookingData.shift} ngày ${formattedDate}`);
        setBookingStep(1);
                            ...
                            <div className="images-gallery">
                                <div className="gallery-item">
                                    <div className="gallery-image-wrapper">
                                        <img src="/images/cs1.avif" alt="Tòa nhà chính" />
                                    </div>
                                    <p>Tòa nhà chính</p>
                                </div>
                                <div className="gallery-item">
                                    <div className="gallery-image-wrapper">
                                        <img src="/images/cs2.jpg" alt="Phòng khám hiện đại" />
                                    </div>
                                    <p>Phòng khám hiện đại</p>
                                </div>
                                <div className="gallery-item">
                                    <div className="gallery-image-wrapper">
                                        <img src="/images/cs3.jpg" alt="Phòng ICU trang bị tối tân" />
                                    </div>
                                    <p>Phòng ICU trang bị tối tân</p>
                                </div>
                                <div className="gallery-item">
                                    <div className="gallery-image-wrapper">
                                        <img src="/images/cs4.jpg" alt="Phòng chờ thoải mái" />
                                    </div>
                                    ...
                                    {specialties.length === 0 ? (
                                        ...
                                        specialties.map((spec, idx) => (
                                            <button key={idx} className="option-btn" onClick={() => { setSelectedSpecialty(spec); setBookingStep(2); }}>
                                                <div className="spec-icon">{getIconForSpecialty(spec)}</div>
                                    ...
                                    <div className="grid-cards">
                                        {filteredDoctors.map(doc => (
                                            <div key={doc.id} className="card-item" onClick={() => { setSelectedDoctor(doc); setBookingStep(3); }}>
                                                ...
                                                <p className="card-schedule">Lịch trống: <strong>{[...new Set(doc.schedule?.map(s => s.date) || [])].length || 0}</strong> ngày</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                <form onSubmit={handleBook} className="booking-form">
                                    <div className="form-group">
                                        ...
                                        <select required className="form-input" 
                                            ...
                                            {/* Lấy danh sách ngày unique từ schedule */}
                                            {[...new Set(selectedDoctor.schedule?.map(s => s.date) || [])].map((date, idx) => (
                    ...
                    <div className="history-container">
                        ...
                        {!appointments || appointments.length === 0 ? (
                            ...
                            <table className="history-table">
                                <thead>
                                    <tr>
                                        <th style={{color: '#000', fontWeight: 700}}>Thời gian</th>
                                        <th style={{color: '#000', fontWeight: 700}}>Chuyên khoa</th>
                                        <th style={{color: '#000', fontWeight: 700}}>Bác sĩ</th>
                                        <th style={{color: '#000', fontWeight: 700}}>Triệu chứng</th>
                                        <th style={{color: '#000', fontWeight: 700}}>Kết quả</th>
                                        <th style={{color: '#000', fontWeight: 700}}>Trạng thái</th>
                                        <th style={{color: '#000', fontWeight: 700}}>Viện phí</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {user && appointments
                                        .filter(a => {
                                            // Lọc lịch hẹn của bệnh nhân hiện tại (so sánh patientId hoặc patientName)
                                            const matchById = (a.patientId === user.patientId || a.patientId === user.id);
                                            const matchByName = a.patientName === user.full_name;
                                            return matchById || matchByName;
                                        })
                                        .sort((a, b) => {
                                            // Sắp xếp theo thời gian mới nhất trước
                                            const timeA = new Date(a.time_booked || a.created_at || 0);
                                            const timeB = new Date(b.time_booked || b.created_at || 0);
                                            return timeB - timeA;
                                        })
                                        .map(app => {
                                            let timeDisplay = app.time_booked || app.time;
                                            // Nếu là dd/MM/yyyy thì chuyển về yyyy-MM-dd
                                            if (timeDisplay && timeDisplay.match(/^\d{2}\/\d{2}\/\d{4}/)) {
                                                const datePart = timeDisplay.split(' ')[0];
                                                const [day, month, year] = datePart.split('/');
                                                const isoDate = `${year}-${month}-${day}`;
                                                timeDisplay = timeDisplay.replace(datePart, isoDate);
                                            }
                                            return (
                                                <tr key={app.id} style={{background: app.status === 'Hoàn thành' ? '#f0fff4' : 'white'}}>
                                                    <td>{timeDisplay}</td>
                                                    <td>{app.specialty}</td>
