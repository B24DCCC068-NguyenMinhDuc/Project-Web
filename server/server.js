...
app.post('/api/appointments', (req, res) => {
    const { patientId, doctor_id, time, symptoms } = req.body;
    const sql = "INSERT INTO appointments (patient_id, doctor_id, time_booked, symptoms) VALUES (?, ?, ?, ?)";
    db.query(sql, [patientId, doctor_id, time, symptoms], (err, result) => {
        if (err) return res.json({ success: false });
